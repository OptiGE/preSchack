      ...
      this._scene.onPointerObservable.add((pointerInfo) => {
          ...
          if (motionController) {
              ...
              case BABYLON.PointerEventTypes.POINTERUP:
                  ...
                  const piece = board.getPiece(startCoord);
                  // eslint-disable-next-line no-case-declarations
                  let promotion = '';

                  if ((piece.pieceValue === 'bp' && endCoord[1] === '1') || (piece.pieceValue === 'wp' && endCoord[1] === '8')) {
                      promotion = 'q';
                  }
                  
                  console.log(`Attempting to make move: ${startCoord}${endCoord}`);
                  api.makeMove(new Move(`${startCoord}${endCoord}${promotion}`));
